...
BaseOfKernelFile equ 08000h ;kernel.bin加载到的位置--段地址
...
LABEL_SEARCH_IN_ROOT_DIR_BEGIN:
  ...
  mov si, KernelFileName  ;ds:si指向文件名"KERNEL BIN"
...
LABEL_DIFFERENT:
  ...
  add di,20h     ;指向下一个目录项
  mov si,KernelFileName ;看下一个目录项中是否是想要的文件
  jmp LABEL_SEARCH_FOR_KERNELBIN 
LABEL_GOTO_NEXT_SECTOR_IN_ROOT_DIR:
  add word [wSectorNo], 1 ;读取下一个扇区
  jmp LABEL_SEARCH_IN_ROOT_DIR_BEGIN
LABEL_NO_KERNELBIN:
  mov dh, 2 ;没有找到,显示数组中第2个字符串"NO KERNEL"
  call DispStr    ;显示字符串
...
LABEL_FILENAME_FOUND:  ;找到KERNEL.BIN后便开始加载这个文件
  mov ax, RootDirSectors
  and di, 0FFE0h ;di->当前目录条目的开始地址(即LOADER.BIN文件的目录项)

  push eax
  mov  eax, [es : di + 01Ch]
  mov  dword [dwKernelSize], eax  ;保存KERNEL.BIN文件的大小
  pop  eax
  ...
  mov ax, BaseOfKernelFile
  mov es, ax       
  mov bx, OffsetOfKernelFile ;数据会加载到BaseOfLoader:OffsetOfLoader指定的位置处
...
LABEL_GOON_LOADING_FILE:
  ...
  mov ah, 0Eh
  mov al, '.'
  mov bl, 0Fh  ;/每读取一个三区就在"Loading"加上.
  int 10h   ;
  pop bx
  pop ax
